...
- 인증 서버와 게이트웨이의 토큰 오류는 같은 모양이지만 `error`에 기계 코드가 들어간다. 목록은 [인증](./10-authentication.md#오류-응답)에 있다.

| HTTP | `error` | 뜻 |
|---|---|---|
| 401 | `invalid_token` | 개인 API 토큰이 없거나·만료·폐기됐다 |
| 403 | `insufficient_scope` | 토큰의 [스코프](./10-authentication.md#스코프)로는 그 경로·메서드를 부를 수 없다 |
| 403 | `pat_cannot_manage_tokens` | 토큰으로 토큰·에이전트 관리 API를 불렀다 |
| 503 | `auth_unavailable` | 게이트웨이가 인증 서버에 닿지 못했다 |
...
| `403` | 권한 없음. 토큰 주인이 그 스페이스·프로젝트에 권한이 없거나, 계정이 `ACTIVE`가 아니거나, 개인 API 토큰의 스코프가 모자라거나(`insufficient_scope`), 토큰으로 토큰 관리 API를 불렀다 | 재시도하지 않는다. 권한(grant)을 받거나, 필요한 스코프로 토큰을 새로 발급하거나, 다른 계정을 쓴다. |
